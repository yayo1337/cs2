...
namespace features::esp::projectile {
	...
	namespace detail {
		...
		constexpr auto icon_radius{ 16.0f };
		constexpr auto ring_radius{ 20.0f };
		constexpr auto ring_thickness{ 2.0f };
		constexpr auto segments{ 48 };
		constexpr auto pulse_speed{ 4.0f };
		constexpr auto pulse_min_scale{ 0.92f };
		constexpr auto pulse_max_scale{ 1.08f };
		constexpr auto arrow_tip_r{ 30.0f };
		constexpr auto arrow_base_r{ 22.0f };
		constexpr auto arrow_spread{ 0.5f };
		constexpr auto arrow_arc_segments{ 12 };
	...
	void overlay::add_landing_indicators( xdraw::draw_list& draw_list )
	{
		...
		for ( const auto& gren : features::misc::g_projectile_trajectory.in_flight( ) )
		{
			...
			const auto draw_damage_text = [ & ]( float cx, float cy )
			{
				if ( !is_he || !cfg.show_damage.value )
				{
					return;
				}

				const auto local = systems::g_local.get( );
				const auto damage = gren.is_enemy
					? features::misc::g_projectile_trajectory.damage_to_local( gren.traj, gren.weapon_hash, local )
					: features::misc::g_projectile_trajectory.max_enemy_damage( gren.traj, gren.weapon_hash, local );
				...
				const auto text = std::string( "-" ) + std::to_string( damage );
				const auto [text_w, text_h] = xdraw::measure_text( text );

				const auto& tc = cfg.text_color.value;
				const auto a = static_cast< std::uint8_t >( static_cast< float >( tc.a ) * state.fade_alpha );
				draw_list.text( std::floorf( cx - text_w * 0.5f ), std::floorf( cy + detail::icon_radius + 6.0f ), text, xdraw::color{ tc.r, tc.g, tc.b, a }, xdraw::text_style::outlined );
			...
			if ( on_screen )
			{
				...
				draw_damage_text( proj.screen.x, proj.screen.y );
				continue;
			}
	...
	void overlay::add_indicator( xdraw::draw_list& draw_list, float cx, float cy, float dir_angle, bool has_arrow, float timer_frac, float alpha, bool is_fire, const settings::esp::projectile::overlay::indicator::group& cfg )
	{
		...
		const auto now = static_cast< float >( std::chrono::duration_cast< std::chrono::milliseconds >( std::chrono::steady_clock::now( ).time_since_epoch( ) ).count( ) ) / 1000.0f;
		const auto pulse_t = std::sinf( now * detail::pulse_speed ) * 0.5f + 0.5f;
		const auto pulse_scale = detail::pulse_min_scale + pulse_t * ( detail::pulse_max_scale - detail::pulse_min_scale );

		// Background ring (subtle, dim)
		{
			const auto& bg = cfg.background_color.value;
			const auto ba = static_cast< std::uint8_t >( static_cast< float >( bg.a ) * alpha * 0.5f );
			draw_list.circle( cx, cy, detail::ring_radius, xdraw::color{ bg.r, bg.g, bg.b, ba }, 1.5f, detail::segments );
		}

		// Countdown ring - depletes clockwise from top as timer runs out
		...
		if ( arc_end > 0.01f )
		{
			...
			for ( auto i = 0; i <= arc_segs; ++i )
			{
				...
				arc_pts.push_back( cx + std::cosf( a ) * detail::ring_radius );
				arc_pts.push_back( cy + std::sinf( a ) * detail::ring_radius );
			}

			const auto span = std::span<const float>( arc_pts.data( ), arc_pts.size( ) );
			draw_list.polyline( span, arc_tint, false, detail::ring_thickness );

			// Pulsing glow on the arc head (the leading edge of the countdown)
			{
				const auto glow_a = static_cast< std::uint8_t >( static_cast< float >( arc_col.a ) * alpha * ( 0.3f + pulse_t * 0.4f ) );
				const auto head_a = -half_pi + arc_end;
				const auto head_x = cx + std::cosf( head_a ) * detail::ring_radius;
				const auto head_y = cy + std::sinf( head_a ) * detail::ring_radius;
				draw_list.circle_filled( head_x, head_y, 2.5f + pulse_t * 1.5f, xdraw::color{ arc_col.r, arc_col.g, arc_col.b, glow_a }, 16 );
			}
			...
			if ( cfg.glow.value )
			{
				...
				glow.polyline( span, xdraw::color{ arc_col.r, arc_col.g, arc_col.b, ga }, false, detail::ring_thickness + 2.0f );
			}
		}

		// Inner filled circle (subtle backdrop for icon)
		{
			const auto& bg = cfg.background_color.value;
			const auto ba = static_cast< std::uint8_t >( static_cast< float >( bg.a ) * alpha * 0.85f );
			const auto inner_r = detail::icon_radius * pulse_scale;
			draw_list.circle_filled( cx, cy, inner_r, xdraw::color{ bg.r, bg.g, bg.b, ba }, detail::segments );
		}
		...
		if ( is_fire )
		{
			...
			if ( ico && ico->texture )
			{
				...
				const auto scale = pulse_scale * 0.9f;
				draw_list.image( std::floorf( cx - iw * 0.5f * scale ), std::floorf( cy - ih * 0.5f * scale ), iw * scale, ih * scale, ico->texture.Get( ), icon_tint );
			}
		}
		else
		{
		...
		// Arrow (off-screen) - clean chevron style
		if ( has_arrow )
		{
