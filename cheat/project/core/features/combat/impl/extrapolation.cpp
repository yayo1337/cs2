...
namespace features::combat {
	...
	{
		return this->extrapolate( pawn, -1, false );
	}

	std::optional<shared::lagcomp::record> shared::lagcomp::extrapolate( std::uintptr_t pawn, int ticks, bool quiet )
	{
		...
		auto ticks_to_extrapolate{ ticks };
		if ( ticks_to_extrapolate <= 0 )
		{
			...
			ticks_to_extrapolate = delta_ticks;
		}
