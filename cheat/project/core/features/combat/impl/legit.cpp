...
namespace features::combat {
	...
	void legit::on_create_move( systems::input::usercmd* cmd )
	{
		...
		// Track if user manually pressed fire BEFORE triggerbot runs, so triggerbot
		// knows not to overwrite manual nospread corrections.
		const auto manual_attack_before_trigger = ( cmd->buttons.value & cstypes::command_buttons::in_attack ) != 0;

		if ( config.manual_nospread.value && manual_attack_before_trigger )
		{
			const auto tick_base = memory::read<std::int32_t>( local.controller + SCHEMA( "CBasePlayerController", "m_nTickBase"_hash ) );
			auto shoot_angles = view_angles;
			shoot_angles.x += aim_punch.x;
			shoot_angles.y += aim_punch.y;
			math::helpers::normalize_angles( shoot_angles );

			const auto corrected = g_shared.find_spread_correction( shoot_angles, tick_base, ctx.inaccuracy, ctx.spread );
			if ( corrected.x != 0.0f || corrected.y != 0.0f || corrected.z != 0.0f )
			{
				if ( const auto base = cmd->csgo_user_cmd.mutable_base( ) )
				{
					...
					{
						va->set_x( corrected.x - aim_punch.x );
						va->set_y( corrected.y - aim_punch.y );
						va->set_z( corrected.z );
					}
				}
				...
				for ( auto i = 0; i < input_history_size; ++i )
				{
					...
					if ( entry )
					{
						...
						{
							angles->set_x( corrected.x - aim_punch.x );
							angles->set_y( corrected.y - aim_punch.y );
							angles->set_z( corrected.z );
						}
					}
				}
			}
		...
		if ( config.aimbot.value )
		{
			...
			if ( shots_fired >= config.aimbot_enable_after.value )
			{
		...
		{
			this->apply_triggerbot( cmd, shoot_position, view_angles, aim_punch, config, local, manual_attack_before_trigger );
		}
	}
	...
	void legit::apply_aimbot( systems::input::usercmd* cmd, const target_result& tgt, const math::vector3& view_angles, const math::vector3& aim_punch, const settings::combat::legitbot::weapon_group& config, const systems::local::snapshot& local )
	{
		...
		if ( !std::isfinite( want_x ) || !std::isfinite( want_y ) )
		{
			this->m_remainder_x = 0.0f;
			this->m_remainder_y = 0.0f;
			return;
		...
		if ( !std::isfinite( deg_per_count ) || deg_per_count <= 0.0f )
		{
			this->m_remainder_x = 0.0f;
			this->m_remainder_y = 0.0f;
			return;
	...
	bool legit::apply_triggerbot( systems::input::usercmd* cmd, const math::vector3& shoot_position, const math::vector3& view_angles, const math::vector3& aim_punch, const settings::combat::legitbot::weapon_group& config, const systems::local::snapshot& local, bool manual_attack_before_trigger )
	{
		const auto& ctx = g_shared.ctx( );
		// Always seeded — deterministic spread for accurate triggerbot.
		const auto seed_mode = true;
			...
			for ( const auto rec : recs )
			{
				...
				if ( seed_mode )
				{
					for ( const auto& hb : hitbox_set )
					{
						...
						// Use same hitboxes as aimbot - no head-only restriction.
						const auto hitbox_idx = static_cast< int >( hb.index );
						const auto hitbox_ok = ( hitbox_idx < 5 ) ? config.hitboxes.values[ hitbox_idx ] : false;
						if ( !hitbox_ok )
						{
							continue;
						}
						...
						if ( candidate_pen.penetrated && candidate_pen.damage < static_cast< float >( config.trigger_min_damage.value ) )
						{
							continue;
						}
		...
		if ( pen.penetrated && pen.damage < static_cast< float >( config.trigger_min_damage.value ) )
		{
			this->m_trigger_pending_pawn = 0;
			return false;
		}
		...
		if ( !seed_mode )
		{
			...
			if ( best_hb && best_hb->bone >= 0 && best_hb->bone < 27 )
			{
				hit_record->apply( );
		...
		const auto input_history_size = cmd->csgo_user_cmd.input_history_size( );
		auto history_angles = math::vector3{ view_angles.x, view_angles.y, 0.0f };

		auto stamp_tick = tick_base;
		if ( config.trigger_nospread.value )
		{
			// Use raw view_angles as the seed base — the server computes the seed
			// from the subtick viewangle, and we'll write (corrected.x - aim_punch.x)
			// to the subtick, so the server sees corrected.x as the shot angle.
			const auto nospread_base = view_angles;

			{
				auto eye_candidates = g_shared.sh( ).get_candidates( );
				if ( eye_candidates.count > 0 )
				{
					const auto& source_eye = eye_candidates.entries[ 0 ];
					...
					std::tie( stamp_tick, std::ignore ) = tick_add( source_eye.player_tick, source_eye.player_frac, source_eye.lerp_ticks_int, source_eye.lerp_ticks_frac );
				}
			}

			const auto corrected = g_shared.find_spread_correction( nospread_base, stamp_tick, g_shared.get_inaccuracy( false ), g_shared.get_spread( ) );
			// Abort if correction couldn't be found (same as ragebot).
			if ( corrected.x == 0.0f && corrected.y == 0.0f && corrected.z == 0.0f )
			{
				return false;
			}
			history_angles = corrected;
		}
		...
		for ( auto i = 0; i < input_history_size; ++i )
		{
			...
			if ( const auto angles = entry->mutable_view_angles( ) )
			{
				// If the user manually pressed fire and manual_nospread is on, preserve
				// the corrections already written by the manual nospread block above.
				if ( config.manual_nospread.value && manual_attack_before_trigger && !config.triggerbot.value )
				{
					// Already written — don't overwrite.
				}
				else if ( config.trigger_nospread.value )
				{
					angles->set_x( history_angles.x - aim_punch.x );
					angles->set_y( history_angles.y - aim_punch.y );
					angles->set_z( history_angles.z );
				}
				else
				{
					// No nospread: write raw view_angles (server will add aim_punch on top).
					angles->set_x( view_angles.x );
					angles->set_y( view_angles.y );
				}
			}
	...
	void legit::draw_fov( xdraw::draw_list& draw_list, const math::vector3& view_angles, const math::vector3& aim_punch, float fov_degrees, const config::col& color, bool rcs_active ) const
	{
		...
		// A stale/uninitialized camera fov (0) during a map transition would make
		// the tangent below zero and produce an infinite circle radius, which can
		// crash the render thread. Bail out instead.
		const auto camera_half_tan = std::tanf( camera_fov_rad * 0.5f );
		if ( !std::isfinite( camera_half_tan ) || camera_half_tan <= 0.0f )
		{
			return;
		}

		const auto radius = std::tanf( aimbot_fov_rad ) / camera_half_tan * ( sw * 0.5f );
		if ( !std::isfinite( radius ) || radius <= 0.0f || radius > sw * 10.0f )
		{
			return;
		}
