...
namespace features::misc {
	...
	void projectile_trajectory::compute_damage_at_endpoint( const trajectory& traj, std::vector<damage_info>& damages, std::uintptr_t weapon_hash, const systems::local::snapshot& local ) const
	{
		damages.clear( );
	...
	int projectile_trajectory::max_enemy_damage( const trajectory& traj, std::uintptr_t weapon_hash, const systems::local::snapshot& local ) const
	{
		std::vector<damage_info> damages{};
		this->compute_damage_at_endpoint( traj, damages, weapon_hash, local );

		auto best{ 0 };
		for ( const auto& dmg : damages )
		{
			best = std::max( best, dmg.damage );
		}

		return best;
	}

	int projectile_trajectory::damage_to_local( const trajectory& traj, std::uintptr_t weapon_hash, const systems::local::snapshot& local ) const
	{
		if ( !traj.valid || traj.end_tick < 0 || weapon_hash != "weapon_hegrenade"_hash )
		{
			return 0;
		}
		...
		const auto trace_start = traj.end_pos + math::vector3{ 0.0f, 0.0f, 1.0f };
		const auto trace = systems::g_tracing.trace( trace_start, center, local.pawn );
		if ( trace.fraction < 0.97f && trace.hit_entity != pawn )
		{
			return 0;
		}
		...
		return std::min( final_damage, health );
	}
	...
	void projectile_trajectory::render_trajectory( xdraw::draw_list& draw_list, const trajectory& traj, float alpha, bool is_thrown, const systems::local::snapshot& local ) const
	{
		...
		std::vector<damage_info> damages{};
		this->compute_damage_at_endpoint( traj, damages, this->m_weapon_hash, local );
