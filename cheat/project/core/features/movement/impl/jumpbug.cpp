...
namespace features::movement {
	...
	void jumpbug::on_create_move( systems::input::usercmd* cmd )
	{
		...
		// Read the live velocity so the trace stays accurate even when
		// prestate was captured a tick early (common on real servers).
		auto velocity = memory::read<math::vector3>( local.pawn + SCHEMA( "C_BaseEntity", "m_vecAbsVelocity"_hash ) );
		if ( velocity.length_sqr() < 0.01f )
		{
			velocity = prestate.velocity;
		}
		...
		// Bias the landing fraction slightly earlier to compensate for the
		// server tick being ahead of the client-side prediction on real servers.
		// Bots run locally so their prediction is frame-perfect; real servers
		// can be 1-2 ticks ahead due to interpolation, which shifts the
		// duck-unduck window and causes missed jumps.
		const auto when = std::clamp( result.fraction - 0.02f, 0.001f, 0.98f );

		this->m_landing_fraction = when;

		buttons.value &= ~( cstypes::command_buttons::in_duck | cstypes::command_buttons::in_jump );
		buttons.value_changed &= ~( cstypes::command_buttons::in_duck | cstypes::command_buttons::in_jump );
