...
namespace features::movement {
	...
	namespace {
		...
		[[nodiscard]] std::optional<float> predict_landing_fraction(
			...
			bool holding_duck )
		{
			if ( prestate.networked_velocity.z > 32.0f )
			{
				return std::nullopt;
			}
			...
			// Let the engine quantize the contact itself.  Rounding beforehand can
			// place the press on the wrong side of a landing and sporadically drop a
			// held hop; jumpbug already uses the unrounded contact fraction.
			return std::clamp( result.fraction, 0.001f, 0.99f );
		}
		...
		void apply_ground_jump( proto::base_usercmd_pb* base )
		{
			const auto subtick_moves = base->mutable_subtick_moves( );

			// Release first so the engine sees a fresh press edge even when the
			// previous command already left the jump button held.
			if ( const auto jump_up = systems::g_input.acquire_subtick_step( subtick_moves ) )
			{
				jump_up->set_button( cstypes::command_buttons::in_jump );
				jump_up->set_pressed( false );
	...
	void bhop::on_create_move( systems::input::usercmd* cmd ) const
	{
		...
		{
			// Jump key held while grounded: the engine needs a fresh press edge
			// to jump. If the landing prediction missed the exact landing tick,
			// the continuously-held key never re-triggers a jump and the hop
			// chain randomly dies - press immediately on the ground tick instead.
			const auto base = cmd->csgo_user_cmd.mutable_base( );
			if ( base )
			{
				apply_ground_jump( base );
			}
		...
		// Always suppress the held jump bit when airborne — the engine will
		// not re-trigger a jump from a continuously-held button on landing.
		// We re-add via subtick below only when a landing is imminent.
		...
		if ( !landing )
		{
			// No landing this tick — jump suppressed, will re-arm next tick.
			return;
		}
