	...
	void airstrafe::on_create_move( systems::input::usercmd* cmd )
	{
		...
		if ( shift_held && in_air )
		{
			...
			if ( subtick_moves )
			{
				...
				if ( step )
				{
					// Seed from the impulses in the basis the command is
					// processed with (antiaim reprojects them when active),
					// otherwise the first delta mixes two different bases.
					const auto base_impulses = features::combat::g_misc.antiaim( ).reproject_moves( prestate.last_movement_impulses );

					step->set_button( 0 );
					step->set_pressed( false );
					step->set_when( 0.0f );
					step->set_analog_forward_delta( forward_move - base_impulses.x );
					step->set_analog_left_delta( left_move - base_impulses.y );
				}
			}
			return;
		}
		...
		// The simulation and subtick deltas must stay in the basis the
		// command is processed with - antiaim already swapped the raw input
		// to that basis, so seed from the reprojected impulses.
		auto last_impulses = features::combat::g_misc.antiaim( ).reproject_moves( prestate.last_movement_impulses );
