...
namespace systems {
	...
	bones::data bones::get( std::uintptr_t entity, std::uint32_t bone_id )
	{
		...
		if ( !cache || static_cast< std::size_t >( bone_id ) >= static_cast< std::size_t >( count ) )
		{
			return result;
		}

		const auto bone = memory::safe_read<data>( cache + static_cast< std::size_t >( bone_id ) * sizeof( data ) ).value_or( data{} );
	...
	std::array<bones::data, 27> bones::get_skeleton( std::uintptr_t entity )
	{
		...
		{
			if ( static_cast< std::size_t >( bone_id ) >= static_cast< std::size_t >( count ) )
			{
				continue;
			}
	...
	std::uintptr_t bones::get_bone_cache( std::uintptr_t entity, int* out_count )
	{
		...
		if ( out_count )
		{
			*out_count = memory::safe_read<int>( game_scene_node + SCHEMA( "CSkeletonInstance", "m_modelState"_hash ) + 0x8c ).value_or( 0 );
		}
