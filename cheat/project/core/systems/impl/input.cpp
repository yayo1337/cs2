...
namespace systems {
	...
	void input::apply( bool suppress_move_alignment )
	{
		...
		if (!suppress_move_alignment && !has_move_subticks (base)) {
	...
	proto::input_history_entry* input::push_input_history( usercmd* cmd, const input_history_params& params ) const
	{
		...
		auto va = entry->mutable_view_angles( );
		if ( !va )
		{
			const auto raw_va = memory::call<void*>(PATTERN (patterns::qangle_alloc), history_field->m_arena );
			if ( raw_va )
			{
				entry->m_view_angles = reinterpret_cast<proto::msg_qangle*>( raw_va );
				va = entry->mutable_view_angles( );
			}
		}
