...
namespace threadpool {
	...
	namespace detail {
		...
		inline pool g_pool {};
		inline std::atomic_bool g_accepting_jobs {false};
		inline std::atomic_uint32_t g_active_jobs {0};
		inline HANDLE g_idle_event {};
		inline std::mutex g_admission_mutex {};

		inline void job_finished () {
			// Hold admission lock while signaling so shutdown cannot CloseHandle
			// the idle event between the last-job transition and SetEvent.
			if (g_active_jobs.fetch_sub (1, std::memory_order_acq_rel) == 1) {
				std::lock_guard lock (g_admission_mutex);
				if (g_idle_event) {
	...
	bool initialize () {
		...
		if (!detail::g_idle_event) {
			detail::g_idle_event = CreateEventW (nullptr, TRUE, TRUE, nullptr);
			if (!detail::g_idle_event) {
				return false;
			}
		}
		detail::g_active_jobs.store (0, std::memory_order_release);
		detail::g_accepting_jobs.store (true, std::memory_order_release);
	...
	bool shutdown( DWORD timeout_ms ) {
		HANDLE idle = nullptr;
		{
			std::lock_guard lock (detail::g_admission_mutex);
			detail::g_accepting_jobs.store (false, std::memory_order_release);
			idle = detail::g_idle_event;
		}
		if (detail::g_active_jobs.load (std::memory_order_acquire) != 0 && idle) {
			if (WaitForSingleObject (idle, timeout_ms) != WAIT_OBJECT_0) {
				return false;
			}
		}
		detail::g_pool = {};
		detail::std_function_job_vtable = 0;
		{
			std::lock_guard lock (detail::g_admission_mutex);
			if (detail::g_idle_event) {
				CloseHandle (detail::g_idle_event);
				detail::g_idle_event = nullptr;
			}
		}
		return detail::g_active_jobs.load (std::memory_order_acquire) == 0;
	}

	job run (std::function<void ()> func, job_priority priority) {
		{
			std::lock_guard lock (detail::g_admission_mutex);
			if (!detail::g_accepting_jobs.load (std::memory_order_acquire)) {
				return {};
			}
			if (detail::g_active_jobs.fetch_add (1, std::memory_order_acq_rel) == 0 && detail::g_idle_event) {
				ResetEvent (detail::g_idle_event);
			}
		}
		auto tracked = [fn = std::move (func)] () mutable {
		...
		auto raw = make_job (std::move (tracked), priority);
		if (!raw) {
			detail::job_finished ();
			return {};
		}
