...
namespace memory {
	...
	namespace detail {
		...
		[[nodiscard]] inline std::size_t capped_strlen (const char* str, std::size_t max_length) noexcept {
			__try {
				std::size_t len {0};
				for (; len < max_length && str [len] != '\0'; ++len)
					;
				return len;
			}
			__except ( EXCEPTION_EXECUTE_HANDLER ) {
				// The string lives outside a valid mapping (e.g. an entity torn
				// down during a map change). Report nothing readable.
				return 0;
			}
		}
...
found_type_descriptor:
	...
	std::string read_string (std::uintptr_t address, std::size_t max_length) {
		...
		auto len = detail::capped_strlen (str_ptr, max_length);
		if (len == 0) {
			return {};
		}
